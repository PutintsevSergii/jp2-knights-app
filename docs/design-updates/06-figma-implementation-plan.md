...
| Formation Roadmap             | Pending direct frame | FR-ROADMAP-002            | Phase 10B read slice implemented: guarded `GET /brother/roadmap`, typed mobile client, demo fixture, and mounted screen model show only the current brother's assigned published roadmap plus latest own submission state.                                                                         | Add brother step submission API/screen flow next; never auto-award degree or expose other brothers' submissions                                                                       |
...
| Brother Announcements List    | Pending direct frame | FR-ANN-001                | Dedicated RN `BrotherAnnouncementsScreen.tsx` renders one-way announcement cards with pinned state, published dates, body copy, top app bar, and bottom nav over the existing guarded list contract. No chat, comments, read receipts, delivery state, candidate-only content, or rosters are exposed. | Add visual QA against a running Expo/native target                                                                                                                                    |
