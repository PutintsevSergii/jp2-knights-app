...
| Create Event                  | Pending direct frame | FR-ADMIN-005              | Admin Lite web event editor now uses the shared Gold/Grey content editor shell with an event-specific form marker over the existing scoped create/detail contract, approval-before-publish lifecycle actions, and no attendee/participant/RSVP controls.                                           | Continue visual QA; keep officer scope and content status workflow                                                                                                                    |
| Event Management List         | Pending direct frame | FR-ADMIN-005              | Admin Lite event list now renders responsive Gold/Grey event cards with type/start/end/location/scope metadata, approval warning support, edit/approve/publish/cancel/archive actions from the existing workflow model, and no attendee/participant/RSVP controls.                                 | Continue visual QA; continue archive/cancel instead of destructive delete                                                                                                             |
