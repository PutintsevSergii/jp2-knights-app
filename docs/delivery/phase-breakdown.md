...
| 5     | Firebase-backed provider adapter verifies tokens/session cookies; enabled Firebase sign-in providers can create/link Idle users without private access; local provider-account linking, logout/session handling, inactive/idle-user blocking, 30-day idle expiry, and mode switching work; fake-provider tests prove replacement path |
| 6     | Officer/Super Admin can enter Admin Lite with scoped navigation; assigned country/region approvers can confirm/reject Idle Firebase sign-ins only inside scope                                                                                                                                                                        |
