...
| **12**    | 🟡 IN PROGRESS | 76%      | ███████████████░░░░░ | Super Admin audit-log API with shared filters/pagination and exact filtered totals, mounted Admin Lite query pass-through, visible audit filter form, shared action preset catalog, and pagination links, Admin Lite audit review route/client validation, candidate request/profile/roadmap submission export-erasure with retention buckets and centralized workflow metadata, candidate profile subject export including local role lifecycle rows, identity access review history, membership history, officer assignment history, roadmap assignment lifecycle metadata without submitted body or attachment data, own event participation intent lifecycle metadata without attendee lists/counts/other user ids, provider account mirrors, safe device-token metadata, and notification preferences, typed Admin Lite privacy clients, self-service device-token revocation, shared retention bucket contract, content publish approval enforcement for prayer/event/announcement/silent-prayer/content-page surfaces, explicit content lifecycle audit actions, Admin Lite approval-before-publish list/create/detail editor workflow across prayer/event/announcement/silent-prayer content, and typed lifecycle mutation helpers                         | Continue retention/erasure/content approval hardening |
...
| ------------------------ | ------- | ------- | ------- | ------- | ------- | ------- | ------- | ------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Lint**                 | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | Full `pnpm quality` lint phase passed after the Phase 12 Admin Lite content editor and shared privacy workflow metadata slices                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **Typecheck**            | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | Full `pnpm quality` typecheck phase passed after the Phase 12 Admin Lite content editor and shared privacy workflow metadata slices                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| **Unit tests (80%)**     | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | `vitest --coverage`: 91.28% statements / 80.11% branches / 91.23% functions / 91.54% lines from the latest full coverage run; targeted shared-types tests cover `ADMIN_PRIVACY_WORKFLOWS` retention/audit/action/path parity, shared validation/Admin Lite/API tests cover candidate profile export role lifecycle rows, identity access review history, membership history, officer assignment history, roadmap assignment lifecycle metadata, own event participation intent lifecycle metadata without attendee lists, provider account mirrors, safe device-token metadata, and notification preferences, Admin Lite content editor model/render/shell tests cover approval-before-publish create/detail routes for prayer/event/announcement/silent-prayer content without participant or attendee actions, and public content repository tests cover the content-page approval filter                                                                                                                                                                                                                                                                                                |
| **Integration tests**    | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | Candidate/brother event read, participation, announcement read, mobile private routing/renderer, role-aware home renderer navigation, provider sign-in session bridge, Expo/Firebase Google provider adapter/config parsing, Brother Prayer Library and Organization Unit Detail mobile route/rendering, mobile public/brother silent-prayer REST list/join screen coverage, mobile Socket.IO heartbeat/leave/reconnect client coverage, Admin Lite Candidate Requests responsive list/detail rendering, admin candidate profile export/erasure client/API coverage including local role lifecycle export, identity access review export, membership export, officer assignment export, roadmap assignment export without submission bodies, own event participation export without participant lists, provider account export, safe device-token export, notification preference export, and erasure cleanup, auth device-token registration/revocation API coverage, admin announcement API/list/editor UI and explicit content lifecycle audit action coverage, Admin Lite content editor route coverage for prayer/event/silent-prayer create/detail approval workflow, Admin Lite privacy clients using shared workflow metadata for candidate request/profile and roadmap submission export/erasure paths, Admin Lite audit-log query pass-through, shared action preset, and exact-total pagination coverage, Admin Lite silent-prayer list API/shell/Next route coverage, notification preference, push recipient dispatch coverage, Phase 10B mobile/admin localization adapter coverage, Admin Lite roadmap review and roadmap submission export/erasure API/client coverage, Super Admin roadmap definition/assignment inspection/creation coverage, Phase 11 silent-prayer public/brother REST/service/admin coverage, and Phase 12 audit-log filter/client plus candidate request export/erasure coverage |
| **Build**                | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | Full `pnpm quality` build phase passed across all apps/libs; `nx run admin:build` also passed after the Phase 12 Admin Lite content editor route slice and shows the new prayer/event/silent-prayer editor routes in the Next manifest                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **OpenAPI generation**   | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | Generated contract includes the Phase 12 audit-log query filters/pagination response, candidate request export/erasure endpoints, candidate profile export including local role lifecycle rows, identity access review history, membership history, officer assignment history, roadmap assignment lifecycle metadata, own event participation intent lifecycle metadata, provider account mirrors, safe device-token lifecycle metadata, and notification preferences plus erasure endpoint, roadmap submission export/erasure endpoints with sensitive-review retention bucket metadata, self-service device-token revocation endpoint, and prior roadmap, silent-prayer, candidate/brother event, participation, announcement, admin announcement, and auth notification endpoints                                                                                                                                                                                                                   |
