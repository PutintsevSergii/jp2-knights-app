...
**Status**: IN PROGRESS (approved V1 scope; mobile shell split complete, roadmap read/submission/review/definition/assignment-inspection and assignment-create slices started)
...
- Phase 10B Super Admin roadmap assignment inspection is in place: guarded `GET /admin/roadmap-assignments` and `GET /admin/roadmap-assignments/:id` routes expose non-archived assignments with assignee, roadmap, organization-unit, lifecycle, and submission-status counts; Admin Lite mounts list/detail routes with typed clients, demo fixtures, responsive rendered screens, and active navigation while submitted body text and assignment mutation remain outside this read-only surface.
- Phase 10B Super Admin roadmap assignment creation API/client support is in place: guarded `POST /admin/roadmap-assignments` creates active assignments only from already-published definitions to eligible candidate/brother users in matching scope, rejects duplicate active/completed assignments for the same definition and scope, writes redacted audit summaries, and exposes typed Admin Lite client support plus create action metadata without adding definition content editing.
