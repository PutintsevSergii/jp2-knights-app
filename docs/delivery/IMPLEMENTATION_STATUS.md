...
| **13**    | 🟡 IN PROGRESS | 25%      | █████░░░░░░░░░░░░░░░ | Pilot readiness planning started with Google Cloud launch docs, Terraform plan, environment/secrets matrix, manual owner task split, workflow timing that keeps Docker/Terraform implementation near the end of V1, Firebase Realtime Database silent-prayer migration plan, June 3, 2026 owner decision that live pilot/production must not provision Redis/Memorystore, API-owned public/brother silent-prayer REST heartbeat/leave contracts for the RTDB migration path, mobile provider-neutral RTDB aggregate-count listener support, API Firebase Admin RTDB presence/publisher support with hashed participant keys and private read grants, deny-by-default RTDB rules baseline, and Firebase emulator rules tests for aggregate public/private count access plus denied client writes                                                                                                                                                                            | Add native-device RTDB validation |
...
- 🟡 Native-device RTDB validation and launch smoke verification
...
**Exit criteria**: 🟡 Planning, API RTDB-prep contracts, mobile listener support, RTDB rules baseline, API RTDB provider/publisher, and Firebase emulator rules tests complete; native-device validation and deployment foundation pending

**Next step**: Add native-device RTDB validation before live Terraform
...
- [ ] Possible improvement after existing items: prepare the Cloud Run + Cloud SQL low-cost deployment optimization slice without Redis, hardening Prisma pool/retry/shutdown behavior, moving production migrations to a standalone Cloud Run Job, and keeping readiness checks shallow on `/api/health`
