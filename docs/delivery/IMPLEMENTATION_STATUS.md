...
- ✅ Mobile `PublicHome`, `AboutOrder`, public `Prayer Library`, public `Events`, and `JoinRequestForm` screens refreshed from the current Google Stitch `JP2 Knights Companion` public mobile screens, with fixed Home/About/Prayers/Events/Join bottom tabs and the Stitch Ecclesia token palette over the existing public DTO/API contracts
...
| **Unit tests (80%)**     | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | `vitest --coverage`: 91.62% statements / 80.67% branches / 91.75% functions / 91.67% lines; Stitch-aligned public mobile renderer paths for Home/About/Prayer Library/Public Events/Join Request, fixed public bottom-tab routing, Phase 12 audit-log API/service/repository filtering, pagination, and exact filtered totals, mounted Admin Lite query pass-through, visible filter form, and server-total-backed pagination links, shared validation, Admin Lite API/model/shell/Next route, candidate request export, candidate profile export, candidate request erasure, candidate profile erasure, self-service device-token revocation, content approval enforcement, Admin Lite approval action workflow including silent-prayer list routing, typed lifecycle mutation helpers, and privacy fallback branches are covered in addition to prior Phase 11 silent-prayer REST/presence/socket/admin/mobile coverage |
...
- Mobile public screens with state models (ready/empty/loading/error/offline/forbidden where supported), fixed public bottom tabs, and Google Stitch-aligned Home/About/Prayer Library/Public Events/Join Request layouts over the same DTOs
