...
| **Unit tests (80%)**     | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | ✅      | `vitest --coverage`: 91.33% statements / 80.34% branches / 91.24% functions / 91.59% lines from the latest full coverage run; targeted shared-types tests cover `ADMIN_PRIVACY_WORKFLOWS` retention/audit/action/path parity, shared validation/Admin Lite/API tests cover candidate profile export role lifecycle rows, identity access review history, membership history, officer assignment history, roadmap assignment lifecycle metadata, own event participation intent lifecycle metadata without attendee lists, provider account mirrors, safe device-token metadata, notification preferences, and admin content approval/publish actor metadata, Admin Lite privacy action model/render tests cover the separate Super Admin privacy-management flag for candidate request/profile/roadmap submission export-erasure controls plus the Super Admin-only workflow overview route, Admin Lite content editor model/render/shell tests cover approval-before-publish create/detail routes plus read-only approval/publish actor evidence for prayer/event/announcement/silent-prayer content without participant or attendee actions, and public content repository tests cover the content-page approval filter                                                                                                                                                                                                                                                                                                |
