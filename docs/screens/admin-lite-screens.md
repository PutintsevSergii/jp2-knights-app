...
- Phase 8 Next Admin follow-up steps are tracked in the delivery dashboard: add Next `dev/start` smoke checks, verify production auth/session cookie behavior through App Router handlers, decide when to retire `dev:http-shell`, and convert framework-neutral renderers to React Server Components only after parity and authorization smoke tests are stable.
