...
| Real time | Implemented baseline is Socket.IO with Redis adapter and Redis TTL presence keys; planned pilot cost-reduction slice migrates silent-prayer aggregate counts to Firebase RTDB behind provider adapters | Redis works across multiple API instances and supports heartbeat counters; RTDB can remove Memorystore idle cost while keeping mobile live updates | Implement the RTDB slice only if API-owned authorization, deny-by-default RTDB rules, private read grants, hashed participant keys, mobile listener cleanup, and aggregate-only tests remain intact |
