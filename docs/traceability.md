...
**Last Updated**: May 24, 2026 (Phases 0–9 complete; Phase 10A V1 Figma/RBAC alignment in progress; Phase 10B roadmap assignment inspection started)
...
- Phase 10B now adds Super Admin read-only roadmap assignment inspection for
  ...
  list/detail screens, and active navigation. Assignment mutation remains
  deferred pending owner-confirmed workflow scope.
...
- Phase 10A implementation should next add pilot Firebase/Google environment
  ...
  Lite routes as responsive web. Phase 10B should next add owner-approved
  roadmap assignment mutation workflow, owner-approved definition mutation
  workflow, or a dedicated brother submission screen flow after confirmation of
  the exact formation wording.
...
| FR-ROADMAP-003 Roadmap Step Submission     | `POST /brother/roadmap/steps/:stepId/submissions`                                                                                                                                                                                             | step detail planned                                                                               | `roadmap_submissions` with optional `attachment_meta`; demo pending brother submission seed                                                                                    | active brother membership required, route/body step id match required, own active assigned published brother roadmap only, scoped organization-unit assignment filtering, published `requiresSubmission` step only, shared DTO attachment policy, duplicate pending submission conflict, no participant lists or auto-degree behavior                                                                                                                                                                                                 |
| FR-ROADMAP-004 Roadmap Approval            | `GET /admin/roadmap-submissions`, `GET/PATCH /admin/roadmap-submissions/:id`, `GET /admin/roadmap-definitions`, `GET /admin/roadmap-definitions/:id`, `GET /admin/roadmap-assignments`, `GET /admin/roadmap-assignments/:id`                  | Admin Lite roadmap submission queue/detail plus roadmap definition and assignment list/detail mounted in Next/App shell | `roadmap_definitions`, `roadmap_stages`, `roadmap_steps`, `roadmap_assignments`, `roadmap_submissions`, audit_logs                                                              | Admin Lite access required for submission review, officer scope constrained by assignment organization unit, Super Admin global review; roadmap definition and assignment inspection are Super Admin-only and read-only; assignment detail exposes submission status metadata only, not submitted body text; list previews only, scoped detail body, shared rejection-comment validation, audited approve/reject decisions with full body redacted, no auto-degree behavior or cross-user rollups; definition/assignment create/edit/status mutations deferred pending owner-confirmed formation wording                                                                                                      |
