...
**Last Updated**: May 25, 2026 (Phases 0–9 complete; Phase 10A/10B in progress; Phase 11 silent-prayer REST/presence foundation started; Cloud Run/Cloud SQL optimization improvement documented)
...
### Current Phase: Phase 10 (V1 Figma/RBAC Alignment + Formation Roadmap) + Phase 11 Test Foundation — In Progress

Implementation is complete through Phase 9 Events, Announcements, and Push. The owner approved pulling the design-update/Figma alignment work into V1 on May 7, 2026. Phase 10 now starts with a 10A Figma/RBAC alignment track before or alongside Phase 10B Formation Roadmap. Phase 11 silent-prayer work has started only as the required presence/counter test foundation before realtime sockets:
...
- Phase 10A implementation should next add pilot Firebase/Google environment
  ...
  remains pending confirmation of the exact formation wording. The next tracked
  technical item is to wire the Phase 11 REST/presence core into Socket.IO and
  a real Redis adapter.
- Phase 11 silent-prayer REST/presence foundation now includes the
  ...
  history. Socket.IO gateway and real Redis adapter wiring remain pending.
...
| FR-PRAYER-002 Public Silent Prayer         | `GET /public/silent-prayer-events`, `POST /public/silent-prayer-events/:id/join`                                                                                                                                                               | public silent prayer                                                                                                                | silent_prayer_events, optional silent_prayer_participation later, Redis-shaped presence store                                                                                   | published active public/family sessions only, archived/cancelled/future/unpublished hidden, anonymous aggregate only, duplicate anonymous session joins do not inflate counters, TTL expiry, explicit leave decrement, multi-instance shared-store counter correctness, no participant identity/session id returned from count/join responses                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| FR-PRAYER-004 Silent Brother Prayer        | `GET /brother/silent-prayer-events`, `POST /brother/silent-prayer-events/:id/join`; socket events pending                                                                                                                                       | brother silent prayer                                                                                                               | silent_prayer_events, Redis-shaped presence store                                                                                                                              | active brother membership required, published active public/family/brother/own-organization-unit sessions only, unrelated scoped sessions hidden, once per authenticated user, heartbeat TTL refresh, reconnect before expiry without count inflation, disconnect expiry, multi-instance shared-store aggregate correctness, no participant list/roster/user id returned before realtime socket wiring                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
