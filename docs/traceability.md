...
- Phase 12 content approval hardening now blocks direct publish transitions for
  ...
  filter.
- Phase 12 approval-retention hardening now rejects admin event and
  silent-prayer updates that would leave a published record without `approvedAt`
  metadata. These failures happen before repository persistence and audit side
  effects.
