...
### Current Phase: Phase 10 (V1 Figma/RBAC Alignment + Formation Roadmap) In Progress; Phase 11 Realtime Foundation Complete; Phase 12 Privacy/Audit Hardening Complete
...
- Next planned live-realtime slice: replace the current Redis/Socket.IO
  silent-prayer realtime path with a Firebase Realtime Database aggregate-count
  provider before Google Cloud pilot, documented in
  [docs/deployment/realtime-db-silent-prayer-migration-plan.md](deployment/realtime-db-silent-prayer-migration-plan.md).
  Owner direction on June 3, 2026 excludes Redis/Memorystore from live pilot and
  production infrastructure unless a future owner-approved scope change reverses
  that decision.
  The API must remain the authorization boundary for event visibility,
  active-window validation, brother membership, and organization-unit scope;
  RTDB may be used only as a read-side aggregate count channel with
  deny-by-default rules, private read grants, server-only writes, hashed
  participant keys, REST heartbeat/leave contracts, provider-neutral adapters,
  and mobile listeners that expose no participant/session/user identity. This
  plan is a cost optimization and does not authorize chat, participant lists,
  analytics, maps, payments, hierarchy-derived permissions, or client-side
...
| FR-PRAYER-004 Silent Brother Prayer        | `GET /brother/silent-prayer-events`, `POST /brother/silent-prayer-events/:id/join`; `/silent-prayer` Socket.IO namespace with `silent-prayer:brother:join`, `silent-prayer:heartbeat`, `silent-prayer:leave`, `silent-prayer:presence`; planned RTDB slice adds brother REST heartbeat/leave contracts and private aggregate-count grants | brother `SilentPrayer` mobile route/screen                                                                                          | silent_prayer_events, implemented Redis-backed presence store; planned Firebase RTDB private aggregate count path and read-grant records | active brother membership required, approved published active public/family/brother/own-organization-unit sessions only, unrelated scoped and unapproved sessions hidden, once per authenticated user, heartbeat TTL refresh, reconnect before expiry without count inflation, disconnect expiry, multi-instance shared-store aggregate correctness, Socket.IO brother join resolves auth through existing session service and delegates scope checks to service, Redis adapter enabled by `REDIS_URL`, mobile bearer-auth REST list/join client validation, demo fixture, aggregate-only renderer tests, mobile Socket.IO auth handshake/join replay/heartbeat/leave/reconnect/error tests, no participant list/roster/user id returned from REST or socket responses; planned RTDB migration requires API-issued per-user/event read grants for private counts because RTDB rules must not replace Postgres-backed membership and organization-unit scope checks |
