...
| `identity_access_reviews`              | Idle external sign-in approval                | `id`, `user_id`, `provider_account_id`, `status`, `scope_organization_unit_id`, `requested_role`, `assigned_role`, `expires_at`, decision metadata                                                                        | indexes user/provider/status/scope/expires; unique pending provider account            | keep decision record               | Admin restricted; included in subject export lifecycle metadata |
...
| `user_roles`                           | Role assignments                              | `id`, `user_id`, `role`, `created_by`, `created_at`, `revoked_at`                                                                                                                                                         | FK user, unique active `(user_id, role)`                                               | revoke                             | Critical audit; included in subject export lifecycle metadata |
...
| `memberships`                          | User-to-unit brother membership               | `id`, `user_id`, `organization_unit_id`, `status`, `current_degree`, `joined_at`, `archived_at`                                                                                                                           | unique active `(user_id, organization_unit_id)`, indexes `organization_unit_id,status` | archive                            | Sensitive; included in subject export lifecycle metadata      |
| `officer_assignments`                  | Officer scope                                 | `id`, `user_id`, `organization_unit_id`, `title`, `starts_at`, `ends_at`                                                                                                                                                  | index `user_id`, `organization_unit_id`                                                | end assignment                     | Critical audit; included in subject export lifecycle metadata |
...
| `events`                               | Public/private events                         | `id`, `title`, `description`, `type`, `start_at`, `end_at`, `location_label`, `visibility`, `target_organization_unit_id`, `status`, approval/publish/cancel metadata                                                        | indexes `start_at,status,visibility,target_organization_unit_id`                       | archive/cancel                     | Location not private address                                  |
| `event_participation`                  | Intent to attend                              | `id`, `event_id`, `user_id`, `intent_status`, `created_at`, `cancelled_at`                                                                                                                                                | unique active `(event_id,user_id)`                                                     | cancel                             | Not attendance tracking; included in subject export as own-intent metadata only |
...
| `roadmap_assignments`                  | User roadmap assignment                       | `id`, `user_id`, `roadmap_definition_id`, `organization_unit_id`, `status`, `assigned_by`, `assigned_at`, `completed_at`                                                                                                  | unique active `(user_id,roadmap_definition_id)`, indexes user/status and scope/status  | archive                            | Private; included in subject export lifecycle metadata without submitted body text |
...
| `silent_prayer_events`                 | Prayer sessions                               | `id`, `title`, `intention`, `starts_at`, `ends_at`, `visibility`, `target_organization_unit_id`, `status`, approval/publish/cancel metadata                                                                                | indexes `visibility,status,starts_at`, `status,published_at`, target unit              | archive/cancel                     | Intention approval; aggregate counter only                    |
...
| `notification_preferences`             | User preferences                              | `id`, `user_id`, `category`, `enabled`                                                                                                                                                                                    | unique `(user_id,category)`                                                            | delete on user erasure if required | Self only; deleted by candidate-profile erasure when required |
| `device_tokens`                        | Push tokens                                   | `id`, `user_id`, `platform`, `token_hash`, `last_seen_at`, `revoked_at`                                                                                                                                                   | unique token hash                                                                      | revoke                             | Sensitive; subject export excludes raw token/hash/last-four  |
...
Subject-access exports include review lifecycle metadata, but access reviews
remain admin restricted and are not exposed to public/mobile read paths.
...
| `events`               | `title text`, `description text`, `type text`, `start_at timestamptz`, `end_at timestamptz`, `location_label text`, `approved_at` | index `start_at,status,visibility,target_organization_unit_id`              | approval metadata required before publish | Avoid private addresses                             |
...
| `silent_prayer_events` | `title text`, `intention text nullable`, `starts_at`, `ends_at`, `cancelled_at`                                    | index time/status/visibility and target organization unit                   | none required             | No participant list exposed                         |
...
| `notification_preferences`    | `id uuid PK`, `user_id uuid FK`, `category notification_category`, `enabled boolean`, timestamps                                                                                                                                                            | unique `(user_id, category)`, index `category,enabled`                                       | Self only; delete on legal erasure when required             |
...
- PostgreSQL row-level security should be enabled for high-risk tables before pilot if the implementation can pass claims into transactions safely. Candidate requests, candidate profiles, identity access reviews, memberships, roadmap submissions, device tokens, and audit logs are the first candidates.
