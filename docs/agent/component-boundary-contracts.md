...
| File pattern | Contract |
| --- | --- |
| `apps/mobile/src/screens/*.tsx` | React Native rendering for a provided screen model, callbacks, and theme tokens. No direct fetch calls, direct storage, role filtering, DTO parsing, or duplicated design-token constants. |
...
| Shared component modules | Add only after two real call sites need the same behavior or visual structure. Components should accept model/action props and avoid owning role-specific data access. |
...
| Shared components/tokens | Shared top app bar, bottom navigation, event card, state views after at least two Figma-matched screens need them |
...
| Shared components/tokens | Shared top app bar, bottom navigation, profile summary card, action row, and state views only after repeated real use |
