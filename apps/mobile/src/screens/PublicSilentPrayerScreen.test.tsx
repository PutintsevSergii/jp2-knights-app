import { describe, expect, it, vi } from "vitest";
import {
  fallbackPublicSilentPrayerJoin,
  fallbackPublicSilentPrayerSessions
} from "../silent-prayer.js";
import { buildPublicSilentPrayerScreen } from "../public-screens.js";
import { PublicSilentPrayerScreen } from "./PublicSilentPrayerScreen.js";

describe("PublicSilentPrayerScreen", () => {
  it("renders aggregate-only public sessions and forwards join actions", () => {
    const onAction = vi.fn();
    const screen = buildPublicSilentPrayerScreen({
      state: "ready",
      response: fallbackPublicSilentPrayerSessions,
      joined: fallbackPublicSilentPrayerJoin,
      runtimeMode: "demo"
    });
    const element = PublicSilentPrayerScreen({ screen, onAction });

    expect(findText(element, "Silent Prayer")).toBe(true);
    expect(findText(element, "Public Silent Prayer")).toBe(true);
    expect(findText(element, "13 praying now")).toBe(true);
    expect(findText(element, "Aggregate counter only")).toBe(true);

    findPressableByLabel(element, "Refresh Counter")?.props.onPress?.();

    expect(onAction).toHaveBeenCalledWith({
      id: "join-silent-prayer",
      label: "Refresh Counter",
      targetRoute: "PublicSilentPrayer",
      targetId: fallbackPublicSilentPrayerSessions.sessions[0]!.id
    });
  });

  it("renders non-ready state copy without session cards", () => {
    const screen = buildPublicSilentPrayerScreen({
      state: "offline",
      runtimeMode: "api"
    });
    const element = PublicSilentPrayerScreen({ screen });

    expect(findText(element, "Offline")).toBe(true);
    expect(findText(element, "Reconnect to refresh silent-prayer sessions.")).toBe(true);
    expect(findText(element, "Public Silent Prayer")).toBe(false);
  });
});

interface TestElement {
  type: string | TestComponent;
  props: {
    accessibilityLabel?: string;
    children?: TestNode;
    onPress?: () => void;
  };
}

type TestNode = TestElement | string | number | boolean | null | undefined | readonly TestNode[];
type TestComponent = (props: TestElement["props"]) => TestNode;

function findPressableByLabel(node: TestNode, label: string): TestElement | undefined {
  const resolved = resolveElement(node);

  if (resolved !== node) {
    return findPressableByLabel(resolved, label);
  }

  if (isTestNodeArray(node)) {
    for (const child of node) {
      const match = findPressableByLabel(child, label);

      if (match) {
        return match;
      }
    }

    return undefined;
  }

  if (!isTestElement(node)) {
    return undefined;
  }

  if (node.type === "Pressable" && node.props.accessibilityLabel === label) {
    return node;
  }

  return findPressableByLabel(node.props.children, label);
}

function findText(node: TestNode, text: string): boolean {
  const resolved = resolveElement(node);

  if (resolved !== node) {
    return findText(resolved, text);
  }

  if (isTestNodeArray(node)) {
    return node.some((child) => findText(child, text));
  }

  if (node === text) {
    return true;
  }

  if (!isTestElement(node)) {
    return false;
  }

  return findText(node.props.children, text);
}

function isTestElement(node: TestNode): node is TestElement {
  return typeof node === "object" && node !== null && "type" in node && "props" in node;
}

function isTestNodeArray(node: TestNode): node is readonly TestNode[] {
  return Array.isArray(node);
}

function resolveElement(node: TestNode): TestNode {
  if (!isTestElement(node) || typeof node.type !== "function") {
    return node;
  }

  return node.type(node.props);
}
